...
class BiometricDeviceConfig(models.Model):
    ...
    def test_device_connection(self):
        conn = None
        zk = ZK(self.device_ip, port=self.port, timeout=10, password=0, force_udp=False, verbose=False)
    ...
    def sync_employees(self):
        conn = None
        zk = ZK(self.device_ip, port=self.port, timeout=10, verbose=True)
                ...
                for attendance in attendances:
                    ...
                    if existing_record:
                        ...
                        vals = {
                            ...
                            'status': str(attendance).split(' ')[6].split(')')[0],
                            'device': self.id,
                            'is_calculated': False
                        }
